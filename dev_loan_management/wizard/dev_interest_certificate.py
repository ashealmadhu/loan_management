...
class dev_interest_certificate(models.TransientModel):
    ...
    _description = "Interest Certificate"
    ...
    def get_lines(self):
        ...
        adv_payment_ids = self.env['dev.advance.payment'].search([('loan_id','=',self.loan_id.id),
                                                                   ('client_id','=',self.client_id.id),
                                                                   ('payment_date','<=',self.end_date),('payment_date','>=',self.start_date),
                                                                   ('state','=','paid')])
        ...
        for installment in adv_payment_ids:
            date = ''
